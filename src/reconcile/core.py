...
class ProviderIndex:
    def __init__(self, participants: tuple[Any, ...]) -> None:
        self._primary_ids = {id(obj) for obj in participants}
        self.all_objs = self._discover(participants)
        self._index: dict[type, list[Any]] = {}
        for obj in self.all_objs:
            for parent in type(obj).__mro__:
                if parent is not object and parent is not BaseModel:
                    self._index.setdefault(parent, []).append(obj)
        self._ns = {cls.__name__: cls for cls in self._index}

    @staticmethod
    def _discover(participants: tuple[Any, ...]) -> tuple[Any, ...]:
    ...
    def resolve(self, requested: type) -> Any:
        candidates = self._index.get(requested, [])
        if len(candidates) > 1:
            primary = [c for c in candidates if id(c) in self._primary_ids]
            if primary:
                candidates = primary
        if len(candidates) == 1:
            return candidates[0]
    ...
    def try_inject(self, fn: Callable[..., Any]) -> Any:
        kw: dict[str, Any] = {}
...
class ReconcileSession:
    def __init__(self, participants: tuple[Any, ...]) -> None:
        ...
        self._providers = ProviderIndex(participants)
        self._states: dict[int, State] = {}
        for obj in self._providers.all_objs:
            if isinstance(obj, BaseModel):
                ...
                self._states[id(obj)] = State(obj, cls, slots)
        self._proxy_classes: dict[type, type] = {}
        self._resolution_stack: list[FieldSlot] = []
    ...
    def _run_cross_validators(self) -> None:
        for state in self._states.values():
            _, cross_validators = _deps(state.cls)
            for dep in cross_validators:
                self._providers.try_inject(dep.fn.__get__(state.obj, state.cls))
    ...
    def _cycle_error(self, slot: FieldSlot) -> ValueError:
        path = self._resolution_stack[self._resolution_stack.index(slot) :] + [slot]
        rendered = " -> ".join(
            f"{self._states[id(s.owner)].cls.__name__}.{s.field_name}" for s in path
        )
    ...
    def _resolve_slot(self, slot: FieldSlot) -> Any:
        ...
        state = self._states[id(slot.owner)]
        slot.result = RESOLVING
        self._resolution_stack.append(slot)
        try:
            result = UNRESOLVED
            for provider in slot.providers:
                result = self._providers.try_inject(
                    provider.fn.__get__(state.obj, state.cls)
                )
    ...
    def _proxy_class_for(self, cls: type[BaseModel]) -> type[BaseModel]:
        ...
        return self._proxy_classes[cls]
